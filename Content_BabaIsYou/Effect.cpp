...
void Effect::Start()
{
	...
	EffectRender->CreateAnimation("WATER_SINK", "Effect.bmp", 98, 104, 0.06f, false);
	EffectRender->CreateAnimation("BABA_SINK", "Effect.bmp", 14, 20, 0.06f, false);
...
void Effect::Update(float _Delta)
{
	...
	if (true == EffectRender->IsAnimation("BABA_WALK")
		|| true == EffectRender->IsAnimation("WALL_WALK"))
	{
		Speed = 50.f;
	...
	if (true == EffectRender->IsAnimation("WATER_SINK") 
		|| true == EffectRender->IsAnimation("BABA_SINK"))
	{
		Speed = 100.f;
		AddPos(Dir * Speed * _Delta);
	}
